...
@permission_classes((permissions.AllowAny,))
@csrf_exempt
def save_settings(request, format=None):

    data = request.data
    try:
        user = data['user']
        whiteTheme = data['data']['theme']
        fontSize = data['data']['font']
    except:
        print("ERROR: в запросе нет нужных данных")
        return HttpResponse(status=400)

    try:
        db_settings = settings.DATABASES['mongo']['db'].sounds.settings
    except:
        print("ERROR: не удалось подключиться к базе данных")
        return HttpResponse(status=500)

    db_settings.remove({'user': user})
    db_settings.insert_one({'user': user, 'theme': whiteTheme, 'font': fontSize})
...
@permission_classes((permissions.AllowAny,))
@csrf_exempt
def load_settings(request, format=None):
    ...
    if db_settings.count({'user': user}) == 0:
        return HttpResponse(status=404)

    cursor = db_settings.find({'user': user})
    data = cursor.next()

    out_dict = {'theme': data['theme'], 'font': data['font']}
    json_out = json.dumps(out_dict)
